...
class AdminAjaxModuleController extends ModuleAdminController{
	...
	public function initContent()
	{
		...
		if ($action == 'save_lang')
		{
			...
			$data[$name] = pSQL(Tools::getValue('value'), true);
		...
		if ($action == 'delete_cf')
		{
			...
			if ($custom_field = Db::getInstance()->getRow($sql))
			{
				Db::getInstance()->delete($table, "id = '".$id."'");
				$id_custom_field = $custom_field['id_custom_field'];
				$table = $module->name.'_custom_field_trans';
				Db::getInstance()->delete($table, 'id_custom_field = '.(int)$id_custom_field);
			}
